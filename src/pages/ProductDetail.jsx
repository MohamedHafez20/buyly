...
import { getProduct, getByCategory, categories } from '../data/products'
...
export default function ProductDetail() {
  ...
  // Fabric and Care detailed recommendations based on category
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <nav className="text-[10px] font-bold tracking-widest uppercase text-neutral-400">
        ...
        <Link to={`/shop?category=${product.category}`} className="hover:text-black transition-colors">{cat?.name}</Link>
        <span className="mx-2 text-neutral-300">/</span>
      ...
      <div className="mt-8 grid gap-12 lg:grid-cols-2">
        ...
        <div className="flex flex-col">
          ...
          <div className="mt-4 flex items-center gap-4 text-xs border-b border-neutral-100 pb-4">
            <StarRating value={product.rating} showValue reviews={product.reviews} />
          ...
          <p className="mt-5 text-sm leading-relaxed text-neutral-500 font-medium">{product.description}</p>
          ...
          <div className="mt-8 pt-6 border-t border-neutral-100">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              ...
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-2 bg-black h-12 px-6 text-[10px] font-bold uppercase tracking-[0.2em] text-white transition-colors duration-250 hover:bg-neutral-800 rounded-none cursor-pointer"
              >
                <Cart size={14} /> Add to Cart
              </button>

              {/* Buy Now */}
              <button
                onClick={() => { addToCart(product, qty); navigate('/checkout') }}
                className="flex-1 border border-neutral-300 bg-white h-12 px-6 text-[10px] font-bold uppercase tracking-[0.2em] text-black hover:bg-neutral-50 transition-colors rounded-none cursor-pointer"
              >
                Buy Now
              </button>
            ...
            <div className="border-b border-neutral-100 py-3">
              ...
              {accordions.care && (
                <div className="mt-3 text-xs leading-relaxed text-neutral-500 font-medium pl-1.5">
                  <p>{getCareInfo(product.category)}</p>
                </div>
              )}
            </div>
